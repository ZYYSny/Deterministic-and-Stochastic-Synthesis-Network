#ifndef CAFFE_REGION_WEIGHTED_LOSS_LAYER_HPP_
#define CAFFE_REGION_WEIGHTED_LOSS_LAYER_HPP_
 ...
 * input to a RegionWeightedLossLayer exactly formulates a linear least squares
...
class RegionWeightedLossLayer : public LossLayer<Dtype> {
 public:
  explicit RegionWeightedLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param), loss_(), count_() {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
            const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline const char* type() const { return "RegionWeightedLoss"; }
  virtual inline int ExactNumBottomBlobs() const { return 3; }
  virtual inline int ExactNumTopBlobs() const { return 1; }
  /**
   * Unlike most loss layers, in the RegionWeightedLossLayer we can backpropagate
 ...
 protected:
  /// @copydoc RegionWeightedLossLayer
   ...
   *
   * Unlike other children of LossLayer, RegionWeightedLossLayer \b can compute
...
#endif  // CAFFE_REGION_WEIGHTED_LOSS_LAYER_HPP_
